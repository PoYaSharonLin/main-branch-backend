...
def create_post(post_id: PositiveInt, comment_item: CommentCreate, db: Session = Depends(get_db), user: UserProfile = Depends(get_current_user)):
...
def update_post(comment_id: PositiveInt, db: Session = Depends(get_db)):
	return comment.read(db=db, comment_id=comment_id)

@comment_router.put('/{comment_id}', response_model=CommentRead)
def update_post(comment_item: CommentUpdate, comment_id: PositiveInt, db: Session = Depends(get_db), user: UserProfile = Depends(get_current_user)):
	return comment.update(db=db, comment_id=comment_id, comment=comment_item, user=user)

@comment_router.delete('/{comment_id}')
def update_post(comment_id: PositiveInt, db: Session = Depends(get_db), user: UserProfile = Depends(get_current_user)):
